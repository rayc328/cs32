...
using ItemType = unsigned long; //using type alias 
const int DEFAULT_MAX_ITEMS = 160; //global variable 

class Sequence
{
private:
    ItemType* mSequence;
    int mSequenceSize; //size of sequence 
    int arrayCapacity; //max capacity of array  

public:

    Sequence(); // Create an empty sequence (i.e., one whose size() is 0).
    Sequence(int arraySize); //constructor to create dynamic array 
    ~Sequence(); //destructor 

    Sequence(const Sequence& sequenceToCopy); //copy constructor 
